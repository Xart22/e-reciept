...
<script type="module">
    ...
    $(document).ready(async function() {
        ...
        function showAutocompleteResults(results) {
            ...
            for (var i = 0; i < results.length; i++) {
                resultList.append(`<a  class="list-group-item list-group-item-action" data-id="${results[i].id}" data-username="${results[i].username}">${results[i].username}</a>`);
            }
            resultList.show();
        }
        ...
        $('#searchInput').on('input', function() {
            ...
            if (inputText.length > 0) {
                results = dataSuggestion.filter(function(item) {
                    return item.username.toLowerCase().indexOf(inputText) !== -1;
                });
            }
   ...
   const urlData = "{{ route('laporan-by-user-api') }}";
